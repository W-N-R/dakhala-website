...
export default function RightSidebar() {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {activeTab && (
          <div className="fixed inset-0 z-50 flex items-center justify-center sm:justify-end sm:pr-16 pointer-events-none">
            <motion.div
              initial={{ opacity: 0, scale: 0.92, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.92, y: 15 }}
              transition={{ type: 'spring', damping: 20, stiffness: 250 }}
              className="flex flex-col overflow-hidden bg-white dark:bg-[#0C132C] border border-border dark:border-white/10 shadow-2xl rounded-3xl w-[calc(100%-2rem)] sm:w-[320px] max-h-[80vh] pointer-events-auto"
            >
            {/* Header */}
